...
class BuildProgress:
    ...
    def _start_timer(self):
        """Start background timer for periodic redraws."""
        self._timer_running = True
        self._timer_thread = threading.Thread(target=self._timer_loop, daemon=True)
        self._timer_thread.start()

    def _timer_loop(self):
        """Periodically redraw the display for smooth timer updates."""
        while self._timer_running and self.enabled:
            now = time.time()
