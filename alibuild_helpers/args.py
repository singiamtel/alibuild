...
DEFAULT_WORK_DIR = (
    os.environ.get("ALIBUILD_WORK_DIR") or os.environ.get("ALICE_WORK_DIR") or "sw"
)
...
def doParseArgs():
  ...
  """
    ...
    subparsers.add_parser(
        "architecture",
        help="display detected architecture",
        description="Display the detected architecture.",
    )
    build_parser = subparsers.add_parser(
        "build", help="build a package", description="Build a package."
    )
    clean_parser = subparsers.add_parser(
        "clean", help="clean up build area", description="Clean up the build area."
    )
    deps_parser = subparsers.add_parser(
        "deps",
        help="generate a dependency graph for a given package",
    ...
    analytics_parser.add_argument(
        "state", choices=["on", "off"], help="Whether to report analytics or not"
    )
    ...
    build_parser.add_argument(
        ...
        help=(
            "Build as if on the specified architecture. When used with --docker, build "
    ...
    build_parser.add_argument(
        ...
        help=(
            "The number of parallel compilation processes to run. "
    ...
    build_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Fetch updates to repositories in MIRRORDIR. Required but nonexistent "
    ...
    build_parser.add_argument(
        ...
        action="append",
        help=(
            "Do not pick up the following packages from a local checkout. "
    ...
    build_parser.add_argument(
        ...
        dest="plugin",
        default="legacy",
        help=("Plugin to use to do the actual build. "),
    )
    build_parser.add_argument(
        "--disable",
        dest="disable",
        ...
        action="append",
        help=(
            "Do not build %(metavar)s and all its (unique) dependencies. "
            "You can specify this option multiple times or separate "
            "multiple arguments with commas."
        ),
    )
    build_parser.add_argument(
        ...
        action="append",
        help=(
            "Always rebuild the following packages from scratch, even if "
    ...
    build_parser.add_argument(
        ...
        default=[],
        action="append",
        metavar="PACKAGE=COMMENT",
        help=(
            "Store COMMENT in the build metadata for PACKAGE. This option "
    ...
    build_parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Display detailed timing information for each build phase",
    )
  ...
  """,
    ...
    build_docker.add_argument(
        ...
        default=None,
        help=(
            "The Docker image to build inside of. Implies --docker. "
  ...
  """,
    ...
    build_remote.add_argument(
        ...
        default="",
        help=(
            "Where to upload newly built packages. Same syntax as --remote-store, "
    ...
    build_dirs.add_argument(
        ...
        help=(
            "The directory where reference git repositories will be cloned. "
    ...
    clean_parser.add_argument(
        ...
        default=detectedArch,
        help=(
            "Clean up build results for this architecture. Default is the current system "
    ...
    clean_dirs.add_argument(
        ...
        default=DEFAULT_CHDIR,
        help=(
            "Change to the specified directory before cleaning up. "
            "Alternatively, set ALIBUILD_CHDIR. Default '%(default)s'."
        ),
    )
    clean_dirs.add_argument(
    ...
    deps_parser.add_argument(
        "package", metavar="PACKAGE", help="Calculate dependency tree for %(metavar)s."
    )
    ...
    deps_parser.add_argument(
        ...
        action="append",
        help=(
            "Assume we're not building %(metavar)s and all its (unique) dependencies. "
  ...
  """,
    ...
    deps_parser.add_argument_group(title="Customise aliBuild directories").add_argument(
        "-c",
        "--config-dir",
        dest="configDir",
        default="alidist",
    ...
    doctor_parser.add_argument(
        ...
        nargs="+",
        help=(
            "Check whether all system requirements of %(metavar)s are satisfied. "
    ...
    doctor_parser.add_argument(
        ...
        default=detectedArch,
        help=(
            "Resolve requirements as if on the specified architecture. When used with "
            "--docker, use a Docker image for the specified architecture. Default is "
            "the current system architecture, which is '%(default)s'."
        ),
    )
  ...
  """,
    ...
    doctor_remote = doctor_parser.add_argument_group(
        title="Re-use prebuilt tarballs",
        description="""\
  Reusing prebuilt tarballs saves compilation time, as common packages need not
  be rebuilt from scratch. rsync://, https://, b3:// and s3:// remote stores
  ...
  """,
    ...
    doctor_dirs.add_argument(
        ...
        default=DEFAULT_CHDIR,
        help=(
            "Change to the specified directory before doing anything. "
            "Alternatively, set ALIBUILD_CHDIR. Default '%(default)s'."
        ),
    )
    doctor_dirs.add_argument(
    ...
    init_parser.add_argument(
        ...
        default=detectedArch,
        help=(
            "Parse defaults using the specified architecture. Default is "
            "the current system architecture, which is '%(default)s'."
        ),
    )
    ...
    init_parser.add_argument(
        ...
        default=".",
        help=(
            "Directory under which to clone the repository of build recipes. "
    ...
    init_parser.add_argument(
        "--dist",
        metavar="[USER/REPO@]BRANCH",
        dest="dist",
        default="",
        type=alidist_string,
        help=(
            "Download the given repository containing build recipes into "
    ...
    init_dirs.add_argument(
        ...
        default=DEFAULT_WORK_DIR,
        help=(
            "The toplevel directory under which builds should be done and "
    ...
    init_dirs.add_argument(
        ...
        "--config-dir",
        dest="configDir",
        default="%(prefix)salidist",
        help=(
            "The directory where build recipes will be placed. '%%(prefix)s' will "
    ...
    version_parser.add_argument(
        ...
        default=detectedArch,
        help=(
            "Display the specified architecture next to the version number. Default is "
            "the current system architecture, which is '%(default)s'."
        ),
    )
...
VALID_ARCHS_RE = (
    "^slc[5-9]_(x86-64|ppc64|aarch64)$|^(ubuntu|ubt|osx|fedora)[0-9]*_(x86-64|arm64)$"
)
...
def finaliseArgs(args, parser):
    ...
    if args.action in ["build", "clean"] and not args.architecture:
        parser.error(
            "Cannot determine architecture. Please pass it explicitly.\n\n"
            + ARCHITECTURE_TABLE
        )

    if (
        args.action == "build"
        and not args.forceUnknownArch
        and not matchValidArch(args.architecture)
    ):
        parser.error(
            "Unknown / unsupported architecture: {architecture}.\n\n{table}"
            "Alternatively, you can use the `--force-unknown-architecture' option.".format(
                table=ARCHITECTURE_TABLE, architecture=args.architecture
            )
        )
    ...
    if args.action in ("build", "doctor", "deps"):
        ...
        if args.docker and not args.dockerImage:
            args.dockerImage = (
                "registry.cern.ch/alisw/%s-builder" % args.architecture.split("_")[0]
            )
    ...
    if "annotate" in args:
        ...
        args.annotate = {
            package: comment
            for package, _, comment in (
                assignment.partition("=") for assignment in args.annotate
            )
        }
    ...
    if args.action in ("build", "doctor"):
        ...
        if (
            args.architecture in S3_SUPPORTED_ARCHS
            and not args.preferSystem
            and not args.no_remote_store
        ):
            args.noSystem = "*"
    ...
    if args.action in ["build", "init"]:
        ...
        if getattr(args, "docker", False):
            args.develPrefix = (
                "%s-%s" % (args.develPrefix, args.architecture)
